...
# 全局参数
datasets_name = "Survival-Spark"
W, S = 40, 10
# 正常数据
normal_mode = "train_CRG"
train_file = rf"HCRL\{datasets_name}\FreeDrivingData.csv"
train_df = pd.read_csv(train_file, on_bad_lines="skip")  # .iloc[0:100000]
# 混合攻击数据
attack_mode = "test_MIX_CRG"
test_file = rf"HCRL\{datasets_name}\process\test_Flooding+Fuzzy+Malfunction.csv"
...
# 2) 构图
# ===============================================================
def build_CRG_graphs(df, out_path, W=40, S=10, is_train=True):
    """
    XIPHOS CRG 构图函数
...
if __name__ == "__main__":
    ...
    # ---------------- Train ----------------
    print("\n=== Load Train CSV ===")
    train_csv_path1 = os.path.join(out_dir, normal_mode, f"raw_{normal_mode}.csv")
    train_df1 = load_and_clean_csv(train_df, train_csv_path1, with_label=False)  # 无标签
    print("\n=== Building Train Graphs ===")
    build_CRG_graphs(train_df1, os.path.join(out_dir, normal_mode), W, S, is_train=True)
